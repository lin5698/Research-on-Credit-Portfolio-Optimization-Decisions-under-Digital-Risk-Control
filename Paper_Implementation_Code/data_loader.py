...
class DataLoader:
    ...
    def split_data(self, df):
        ...
        """
        # Dynamic split: 80% Training, 20% Optimization
        # This removes the hardcoded 123/32 split from the paper reproduction
        train_frac = 0.8
        train_df = df.sample(frac=train_frac, random_state=42)
        optimize_df = df.drop(train_df.index)
        
        print(f"Data Split: Training ({len(train_df)}), Optimization ({len(optimize_df)})")
