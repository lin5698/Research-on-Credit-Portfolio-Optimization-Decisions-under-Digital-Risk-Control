...
class LoanPortfolioOptimizer:
    ...
    def objective_functions(self, solution):
        ...
        income = np.sum(amounts[active_idx] * rates[active_idx] * (1 - self.pd[active_idx]))
        el = np.sum(amounts[active_idx] * self.pd[active_idx])
        
        # Economic Capital (EC) simplified as Unexpected Loss (UL)
        ul = np.sum(amounts[active_idx] * np.sqrt(self.pd[active_idx] * (1 - self.pd[active_idx])))
        ...
        losses = np.dot(defaults, active_amounts)
